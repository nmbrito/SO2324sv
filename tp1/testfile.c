...
int main(int argc, char *argv[])
{
    ...
    int *arraySubValues = (int *) malloc(sizeof(int) * (arraySize / numberProcesses)); // ou ((arraySize / numberProcesses) + 1)
        ...
        else if(forker == CHILD) // Child work
        {
            ...
            for (long i = 0; i < (slices+addLeftover); i++)
            {
                if (arrayValues[i] >= LOWER_LIMIT && arrayValues[i] <= UPPER_LIMIT)
                {
                    ...
                    arraySubValues[svCount[0]++] = arrayValues[i];
                }
            }
                //svCount[0] = count;
            ...
            write(pipesfds[WRITE], svCount, sizeof(long)); //se a estratégia do -1 for usada, isto desaparece
    ...
    //APAGAR:int *arraySubValuesMaster = (int *) malloc(sizeof(int) * arraySize);
    //arraySubValues = realloc(arraySubValues, (sizeof(int) * arraySize));
    ...
    for(int closeProcesses = 0; closeProcesses < numberProcesses; closeProcesses++)
    {
        ...
        // Verificar arraySubValues[v_sz-1]
        // Reposicionar arraySubValues
        // Juntar esse valor a um somatório
    ...
    arraySubValues = realloc(arraySubValues, (sizeof(int) * countnumber));
